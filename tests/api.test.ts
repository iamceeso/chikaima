...
test("api.getProfile attaches auth headers and parses JSON", async () => {
  ...
  assert.equal((capturedRequest?.headers as Headers).get("Authorization"), "Bearer token-123");
  assert.equal(payload.id, "user-1");
});

test("api.deleteConversation resolves undefined for 204 responses", async () => {
...
test("api.getWorkspaceSettings surfaces backend detail messages", async () => {
  global.fetch = async () =>
...
test("api request rethrows non-timeout fetch failures", async () => {
  global.fetch = async () => {
  ...
  await assert.rejects(
    () => api.getPublicWorkspaceSettings(),
    (error: unknown) => error instanceof Error && error.message === "Socket closed",
  );
});

test("api.streamChat emits metadata tokens done and error events", async () => {
  ...
  await api.streamChat(
    ...
    { content: "Hi there" },
    {
      onMetadata: (metadata: Record<string, unknown>) => seen.push(`metadata:${String(metadata.conversation_id)}`),
      onToken: (text: string) => seen.push(`token:${text}`),
...
test("api.streamChat handles chunked events and fallback error parsing", async () => {
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      const encoder = new TextEncoder();
  ...
  assert.deepEqual(seen, ["token:Split", "error:Streaming failed"]);
});

test("api.streamChat surfaces non-ok streaming responses", async () => {
  ...
  await assert.rejects(
    () => api.streamChat("token-123", { content: "Hi" }),
    (error: unknown) => error instanceof Error && error.message === "Streaming request failed",
  );
});

test("api request helpers cover auth, json, blob, and upload wrappers", async () => {
...
test("api request surfaces non-json backend errors and requestBlob timeout", async () => {
  let call = 0;
  ...
  await assert.rejects(
    () => api.getDashboard("token-1"),
    (error: unknown) => error instanceof Error && error.message === "Gateway exploded",
  );
  await assert.rejects(
    () => api.getUsers({ token: "token-1" }),
    (error: unknown) => error instanceof Error && error.message === "{not valid json",
  );
  await assert.rejects(
    () => api.getAssetFile("token-1", "video", "video-1"),
    (error: unknown) => error instanceof Error && error.message === "Request timed out. Check that the backend is running.",
  );
});

test("api request falls back to the original json body when detail is missing", async () => {
  global.fetch = async () =>
    new Response(JSON.stringify({ message: "No detail field" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
...
test("api.getAssetFile surfaces non-timeout and non-ok blob failures", async () => {
  let call = 0;
  ...
  await assert.rejects(
    () => api.getAssetFile("token-1", "document", "doc-1"),
    (error: unknown) => error instanceof Error && error.message === "Socket closed",
  );
  await assert.rejects(
    () => api.getAssetFile("token-1", "document", "doc-1"),
    (error: unknown) => error instanceof Error && error.message === "Request failed",
  );
});

test("api private request helpers preserve explicit fetch options and cover blob authHeader errors", async () => {
  const requests: Array<{ url: string; init?: RequestInit }> = [];
  ...
  await assert.rejects(
    () => apiTestUtils.requestBlob("/blob-error", { authHeader: "Basic abc123" }),
    (error: unknown) => error instanceof Error && error.message === "Socket closed",
  );
  const blob = await apiTestUtils.requestBlob("/blob-ok", { authHeader: "Basic abc123" });

  assert.equal(await blob.text(), "blob-content");
  assert.equal(requests[0]?.url, "https://api.example.com/custom");
  assert.equal(requests[0]?.init?.cache, "reload");
  assert.equal(requests[0]?.init?.signal, customSignal);
  assert.equal((requests[0]?.init?.headers as Headers).get("Content-Type"), "application/custom+json");
  assert.equal((requests[0]?.init?.headers as Headers).get("Authorization"), null);
  assert.equal((requests[1]?.init?.headers as Headers).get("Authorization"), "Basic abc123");
  assert.equal((requests[2]?.init?.headers as Headers).get("Authorization"), "Basic abc123");
});

test("api helper timeouts invoke abort callbacks for request and requestBlob", async () => {
  ...
  global.clearTimeout = ((timeoutId: unknown) => {

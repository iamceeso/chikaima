...
class AuthServiceTests(unittest.TestCase):
    def test_init_builds_user_repository(self) -> None:
        ...
        with patch("app.services.auth_service.UserRepository", return_value="repo") as repository:
    ...
    def test_register_rejects_duplicate_email(self) -> None:
        ...
        with self.assertRaises(HTTPException) as context:
            service.register(SimpleNamespace(email="taken@example.com", full_name="Taken", password="password123"))
    ...
    def test_create_user_rejects_duplicate_email(self) -> None:
        ...
        with self.assertRaises(HTTPException) as context:
            service.create_user(
                ...
                SimpleNamespace(email="taken@example.com", full_name="Taken", password="password123", is_superuser=False, is_active=True),
            )
    ...
    def test_create_user_persists_new_user(self) -> None:
        ...
        with patch("app.services.auth_service.hash_password", return_value="hashed-password"):
            user = service.create_user(
                SimpleNamespace(is_superuser=True),
                SimpleNamespace(email="new@example.com", full_name="New User", password="password123", is_superuser=False, is_active=True),
            )
    ...
    def test_update_user_rejects_non_admin_missing_user_and_duplicate_email(self) -> None:
        service = object.__new__(AuthService)
        service.users = SimpleNamespace(get=lambda user_id: None, get_by_email=lambda email: object(), count_superusers=lambda: 2)

        with self.assertRaises(HTTPException) as non_admin_context:
            service.update_user(SimpleNamespace(is_superuser=False), "user-1", SimpleNamespace(model_dump=lambda exclude_unset=True: {}))
        self.assertEqual(non_admin_context.exception.status_code, 403)

        with self.assertRaises(HTTPException) as missing_context:
            service.update_user(SimpleNamespace(is_superuser=True), "user-1", SimpleNamespace(model_dump=lambda exclude_unset=True: {}))
        self.assertEqual(missing_context.exception.status_code, 404)

        existing = SimpleNamespace(id="user-1", email="old@example.com", is_superuser=False, is_active=True)
        service.users = SimpleNamespace(get=lambda user_id: existing, get_by_email=lambda email: object(), count_superusers=lambda: 2)

        with self.assertRaises(HTTPException) as duplicate_context:
            service.update_user(
                SimpleNamespace(is_superuser=True),
                "user-1",
                SimpleNamespace(model_dump=lambda exclude_unset=True: {"email": "taken@example.com"}),
            )
    ...
    def test_update_user_prevents_disabling_last_admin_and_applies_updates(self) -> None:
        admin = SimpleNamespace(id="user-1", email="admin@example.com", full_name="Admin", hashed_password="old", is_superuser=True, is_active=True)
        service = object.__new__(AuthService)
        service.users = SimpleNamespace(get=lambda user_id: admin, get_by_email=lambda email: None, count_superusers=lambda: 1)
        service.db = SimpleNamespace(add=lambda _item: None, commit=lambda: None, refresh=lambda _item: None)

        with self.assertRaises(HTTPException) as inactive_context:
            service.update_user(
                SimpleNamespace(is_superuser=True),
                "user-1",
                SimpleNamespace(model_dump=lambda exclude_unset=True: {"is_active": False}),
            )

        self.assertEqual(inactive_context.exception.status_code, 400)

        user = SimpleNamespace(id="user-2", email="user@example.com", full_name="User", hashed_password="old", is_superuser=False, is_active=True)
        commits: list[str] = []
        refreshed: list[object] = []
        service.users = SimpleNamespace(get=lambda user_id: user, get_by_email=lambda email: None, count_superusers=lambda: 2)
        service.db = SimpleNamespace(add=lambda item: refreshed.append(item), commit=lambda: commits.append("commit"), refresh=lambda item: refreshed.append(("refresh", item)))
    ...
    def test_update_user_keeps_existing_fields_when_updates_are_empty(self) -> None:
        ...
        service = object.__new__(AuthService)
        service.users = SimpleNamespace(get=lambda user_id: user, get_by_email=lambda email: None, count_superusers=lambda: 2)
        service.db = SimpleNamespace(add=lambda _item: None, commit=lambda: commits.append("commit"), refresh=lambda item: refreshed.append(item))
    ...
    def test_login_rejects_when_authentication_disabled_and_returns_tokens_on_success(self) -> None:
        user = SimpleNamespace(id="user-1", hashed_password="hashed")
        service = make_auth_service(user)
        ...
        with patch("app.services.auth_service.WorkspaceService") as workspace_service:
            ...
            with self.assertRaises(HTTPException) as disabled_context:
                service.login(SimpleNamespace(email="person@example.com", password="password123"))
        ...
        with (
            ...
            workspace_service.return_value.get_or_create.return_value = SimpleNamespace(authentication_enabled=True)
            logged_in_user, access_token, refresh_token = service.login(SimpleNamespace(email="person@example.com", password="password123"))
    ...
    def test_refresh_rejects_disabled_authentication_and_invalid_tokens(self) -> None:
        ...
        with (
            patch("app.services.auth_service.WorkspaceService") as workspace_service,
            patch("app.services.auth_service.decode_token", side_effect=ValueError("bad token")),
        ):
            workspace_service.return_value.get_or_create.return_value = SimpleNamespace(authentication_enabled=True)
    ...
    def test_request_password_reset_returns_generic_message_for_missing_user(self) -> None:
        service = make_auth_service(None)

        with patch("app.services.auth_service.app_settings", SimpleNamespace(is_production=False)):
    ...
    def test_request_password_reset_never_returns_token(self) -> None:
        ...
        self.assertIn("Password reset token for person@example.com: reset-token", logs.output[0])

    def test_request_password_reset_handles_unreachable_log_branch_without_returning_token(self) -> None:
        user = SimpleNamespace(id="user-1", email="person@example.com")
        service = make_auth_service(user)
    ...
    def test_confirm_password_reset_rejects_missing_user_and_updates_password(self) -> None:
        service = make_auth_service(None)
        service.db = SimpleNamespace(add=lambda item: None, commit=lambda: None)

        with patch("app.services.auth_service.decode_token", return_value={"sub": "user-1"}):
            with self.assertRaises(HTTPException) as missing_context:
                service.confirm_password_reset(SimpleNamespace(token="reset-token", new_password="new-password-123"))
        ...
        commits: list[str] = []
        service = make_auth_service(user)
        service.db = SimpleNamespace(add=lambda item: added.append(item), commit=lambda: commits.append("commit"))

        with (
            patch("app.services.auth_service.decode_token", return_value={"sub": "user-1"}),
            patch("app.services.auth_service.hash_password", return_value="new-hash"),
        ):
            response = service.confirm_password_reset(SimpleNamespace(token="reset-token", new_password="new-password-123"))

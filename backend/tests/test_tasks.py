...
class FakeQuery:
    def __init__(self, *, first_result: object | None = None, all_result: list[object] | None = None) -> None:
...
class FlakyJobLookupDB(FakeDB):
    def __init__(self, job: object | None, resource: object | None, *, return_job_on_second_lookup: bool, return_resource_on_second_lookup: bool) -> None:
...
class WorkerTaskTests(unittest.TestCase):
    def test_task_wrappers_delegate_to_shared_processor(self) -> None:
        with patch("app.workers.tasks._process_resource_job", return_value={"status": "ok"}) as process:
    ...
    def test_process_resource_job_returns_failure_when_job_is_missing(self) -> None:
        ...
        self.assertEqual(result, {"job_id": "missing", "status": "failed", "message": "Job not found"})
        self.assertTrue(db.closed)

    def test_process_resource_job_marks_job_failed_when_resource_is_missing(self) -> None:
        ...
        self.assertEqual(result, {"job_id": "job-1", "status": "failed", "message": "Resource not found"})
    ...
    def test_process_resource_job_raises_when_audio_transcript_is_empty(self) -> None:
        job = SimpleNamespace(id="job-audio", resource_id="audio-1", status="pending", error_message=None)
        resource = SimpleNamespace(
            id="audio-1",
            name="empty.wav",
    ...
    def test_process_resource_job_failure_does_not_update_resource_when_job_has_no_resource_id(self) -> None:
        job = SimpleNamespace(id="job-9", resource_id=None, status="pending", error_message=None)
        resource = SimpleNamespace(
            id="audio-1",
            name="call.wav",
            file_path="/tmp/call.wav",
        ...
        with (
            patch("app.workers.tasks.SessionLocal", return_value=db),
            patch("app.workers.tasks.WhisperTranscriptionService", side_effect=RuntimeError("boom")),
        ):
    ...
    def test_process_resource_job_failure_skips_updates_when_follow_up_lookup_cannot_find_job(self) -> None:
        job = SimpleNamespace(id="job-10", resource_id="audio-1", status="pending", error_message=None)
        resource = SimpleNamespace(
            id="audio-1",
            name="call.wav",
            file_path="/tmp/call.wav",
        ...
        db = FlakyJobLookupDB(job, resource, return_job_on_second_lookup=False, return_resource_on_second_lookup=True)
    ...
    def test_process_resource_job_failure_skips_resource_update_when_follow_up_lookup_cannot_find_resource(self) -> None:
        job = SimpleNamespace(id="job-11", resource_id="audio-1", status="pending", error_message=None)
        resource = SimpleNamespace(
            id="audio-1",
            name="call.wav",
            file_path="/tmp/call.wav",
        ...
        db = FlakyJobLookupDB(job, resource, return_job_on_second_lookup=True, return_resource_on_second_lookup=False)
    ...
    def test_process_resource_job_completes_video_when_transcription_returns_empty_text(self) -> None:
        ...
        with (
            ...
            patch(
                ...
                return_value={"summary": "summary", "key_points": [], "action_items": [], "chapters": []},
            ),
    ...
    def test_process_resource_job_completes_video_when_transcription_returns_text(self) -> None:
        job = SimpleNamespace(id="job-4", resource_id="video-2", status="pending", error_message=None, result={})
        resource = SimpleNamespace(
            id="video-2",
            name="demo.mp4",
        ...
        with (
            ...
            transcription_service.return_value.transcribe_media.return_value = "Spoken words"
    ...
    def test_process_resource_job_handles_document_extraction_locally(self) -> None:
        job = SimpleNamespace(id="job-3", resource_id="doc-1", status="pending", error_message=None, result={})
        resource = SimpleNamespace(
            id="doc-1",
            name="notes.txt",
    ...
    def test_process_resource_job_uses_existing_document_chunks_without_rebuilding(self) -> None:
        job = SimpleNamespace(id="job-5", resource_id="doc-2", status="pending", error_message=None, result={})
        resource = SimpleNamespace(
            id="doc-2",
            name="slides.pptx",
        ...
        with (
            ...
            patch("app.workers.tasks.EmbeddingsService") as embeddings_service,
            patch("app.workers.tasks._build_chunks", side_effect=AssertionError("should not rebuild chunks")),
        ):
    ...
    def test_upsert_transcript_creates_new_transcript(self) -> None:
        ...
        self.assertEqual(transcript.segments, [{"speaker": "system", "text": "Transcript body"}])
        self.assertEqual(transcript.status, "completed")
        self.assertEqual(db.added, [transcript])
        self.assertEqual(db.flushes, 1)

    def test_upsert_transcript_updates_existing_transcript_and_clears_segments_for_empty_content(self) -> None:
    ...
    def test_upsert_summary_artifacts_creates_and_updates_records(self) -> None:
        existing_summary = SimpleNamespace(summary_type="summary", content="old", data={"x": 1}, status="pending")
        db = FakeDB(job=None, resource=None)
    ...
    def test_generate_summary_bundle_returns_fallback_for_blank_and_failing_content(self) -> None:
        resource = SimpleNamespace(name="Notes.pdf", user_id="user-1")

        blank = tasks._generate_summary_bundle(SimpleNamespace(), resource, "document", "   ")
        ...
        with patch("app.workers.tasks.LLMService", side_effect=RuntimeError("boom")):
            failed = tasks._generate_summary_bundle(SimpleNamespace(), resource, "document", "Some content")

        self.assertIn("Notes.pdf contains extracted document content.", failed["summary"])

    def test_generate_summary_bundle_populates_video_fields_and_uses_fallback_values_when_needed(self) -> None:
        replies = iter(
            [
                "",
                "   ",
        ...
        with patch("app.workers.tasks.LLMService", return_value=llm):
            bundle = tasks._generate_summary_bundle(SimpleNamespace(), resource, "video", "Transcript body")

        self.assertIn("Walkthrough.mp4 contains extracted video content.", bundle["summary"])
    ...
    def test_generate_summary_bundle_populates_audio_fields_without_video_chapters(self) -> None:
        replies = iter(
            [
                "Summary text",
                "- Point one\n- Point two",
        ...
        with patch("app.workers.tasks.LLMService", return_value=llm):
            bundle = tasks._generate_summary_bundle(SimpleNamespace(), resource, "audio", "Transcript body")
    ...
    def test_generate_summary_bundle_populates_document_fields_without_audio_or_video_branches(self) -> None:
        replies = iter(
            [
                "Document summary",
                "- Point one",
        ...
        with patch("app.workers.tasks.LLMService", return_value=llm):
            bundle = tasks._generate_summary_bundle(SimpleNamespace(), resource, "document", "Transcript body")
    ...
    def test_fallback_summary_bundle_and_parse_bullets_cover_supported_formats(self) -> None:
        with_text = tasks._fallback_summary_bundle("notes.txt", "document", "One   two   three")
        ...
        self.assertEqual(tasks._parse_bullets("- first\n* second\nthird"), ["first", "second", "third"])

    def test_update_resource_fields_sets_expected_attributes(self) -> None:
        audio = tasks.AudioAsset(user_id="user-1", name="call.wav", file_path="/tmp/call.wav")
        video = tasks.Video(user_id="user-1", name="demo.mp4", file_path="/tmp/demo.mp4")
        document = tasks.Document(user_id="user-1", name="notes.pdf", file_path="/tmp/notes.pdf", mime_type="application/pdf")
        bundle = {"summary": "Summary", "chapters": ["Intro"], "action_items": ["Follow up"]}
    ...
    def test_resolve_asset_type_covers_supported_document_types(self) -> None:
        ...
        fallback = SimpleNamespace(name="archive.bin", mime_type="application/octet-stream")

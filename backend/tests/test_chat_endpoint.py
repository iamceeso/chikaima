...
def make_message(
    message_id: str, conversation_id: str, role: str, content: str
) -> SimpleNamespace:
...
class ChatEndpointTests(unittest.TestCase):
    def test_conversation_endpoints_validate_service_results(self) -> None:
        ...
        with patch("app.api.v1.endpoints.chat.ChatService") as chat_service:
            ...
            created = create_conversation(
                SimpleNamespace(title="Chat"), db, current_user
            )
            added = add_message(
                "conv-1",
            ...
            edited = edit_message(
                "msg-1", SimpleNamespace(content="Updated"), db, current_user
            )
            regenerated = regenerate_message(
                SimpleNamespace(message_id="msg-1"), db, current_user
            )
        ...
        chat_service.return_value.delete_conversation.assert_called_once_with(
            "user-1", "conv-1"
        )
    ...
    def test_stream_chat_returns_pending_notice_stream_for_new_conversation(
        self,
    ) -> None:
        db = SimpleNamespace(
        ...
        service = SimpleNamespace(
            ...
            _resolve_chat_model_and_provider=lambda *args, **kwargs: (
                SimpleNamespace(model_key="gpt-4o-mini"),
                SimpleNamespace(provider_type="openai"),
            ),
        ...
        with patch(
            "app.api.v1.endpoints.chat.ChatService", return_value=service
        ), patch(

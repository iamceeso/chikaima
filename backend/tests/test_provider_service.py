...
class ProviderServiceTests(unittest.TestCase):
    ...
    def test_init_and_list_for_user_delegate_to_repository(self) -> None:
        ...
        with patch(
            "app.services.provider_service.ProviderRepository", return_value="repo"
        ) as repository:
    ...
    def test_openrouter_model_filter_excludes_non_chat_endpoints(self) -> None:
        ...
        self.assertFalse(
            _should_include_openrouter_model("openai/text-embedding-3-small")
        )
    ...
    def test_join_api_url_avoids_duplicate_suffix(self) -> None:
        self.assertEqual(
            _join_api_url("https://api.example.com/v1/models", "models"),
            "https://api.example.com/v1/models",
        )
        self.assertEqual(
            _join_api_url("https://api.example.com/v1", "models"),
            "https://api.example.com/v1/models",
        )
    ...
    def test_load_provider_models_reencrypts_legacy_api_keys(self) -> None:
        ...
        service.db = type(
            "DB", (), {"add": lambda _self, item: added_items.append(item)}
        )()
        ...
        with patch.object(
            service,
            "_fetch_openai_models",
            return_value=CURATED_PROVIDER_MODELS["openai"],
        ) as fetcher:
        ...
        self.assertTrue(
            provider.encrypted_config["api_key"].startswith(ENCRYPTED_VALUE_PREFIX)
        )
        self.assertEqual(added_items, [provider])

    def test_update_does_not_reload_models_when_connection_fields_are_unchanged(
        self,
    ) -> None:
        provider = type(
            "Provider",
        ...
        service.providers = type(
            "Repo", (), {"get": lambda _self, provider_id: provider}
        )()
        service.db = type(
            "DB",
            (),
        ...
        with (
            patch.object(service, "_load_provider_models") as loader,
    ...
    def test_update_rejects_missing_provider_and_reloads_models_when_connection_fields_change(
        self,
    ) -> None:
        service = object.__new__(ProviderService)
        ...
        with self.assertRaises(HTTPException) as context:
            service.update(
                ...
                "missing",
                SimpleNamespace(
                    name=None, base_url=None, is_enabled=None, config=None, api_key=None
                ),
            )
        ...
        with (
            ...
            patch.object(service, "_replace_provider_models") as replacer,
            patch(
                "app.services.provider_service.secret_manager.encrypt",
    ...
    def test_replace_provider_models_preserves_existing_default_and_assigns_first_global_default(
        self,
    ) -> None:
    ...
    def test_load_provider_models_dispatches_all_provider_types(self) -> None:
        ...
        with (
            patch.object(
                service, "_fetch_openai_models", return_value=["openai"]
            ) as openai_fetcher,
            patch.object(
                service, "_fetch_anthropic_models", return_value=["anthropic"]
            ) as anthropic_fetcher,
            patch.object(
                service, "_fetch_gemini_models", return_value=["gemini"]
            ) as gemini_fetcher,
            patch.object(
                service, "_fetch_ollama_models", return_value=["ollama"]
            ) as ollama_fetcher,
        ):
            self.assertEqual(
                service._load_provider_models(
                    SimpleNamespace(
                        provider_type="openai", encrypted_config={}, base_url=None
                    ),
                    "sk",
                ),
                ["openai"],
            ...
            self.assertEqual(
                service._load_provider_models(
                    SimpleNamespace(
                        provider_type="anthropic", encrypted_config={}, base_url=None
                    ),
                    "sk",
                ),
                ["anthropic"],
            ...
            self.assertEqual(
                service._load_provider_models(
                    SimpleNamespace(
                        provider_type="gemini", encrypted_config={}, base_url=None
                    ),
                    "sk",
                ),
                ["gemini"],
            ...
            self.assertEqual(
                service._load_provider_models(
                    SimpleNamespace(
                        provider_type="ollama", encrypted_config={}, base_url=None
                    ),
                    "sk",
                ),
                ["ollama"],
            ...
            self.assertEqual(
                service._load_provider_models(
                    SimpleNamespace(
                        provider_type="openrouter", encrypted_config={}, base_url=None
                    ),
                    "sk",
                ),
                ["openai"],
            ...
            self.assertEqual(
                service._load_provider_models(
                    SimpleNamespace(
                        provider_type="litellm", encrypted_config={}, base_url=None
                    ),
                    "sk",
                ),
                ["openai"],
    ...
    def test_fetch_openai_models_uses_curated_fallbacks_when_unavailable(self) -> None:
        ...
        self.assertEqual(
            service._fetch_openai_models(provider, None),
            CURATED_PROVIDER_MODELS["openrouter"],
        )
        self.assertEqual(
            service._fetch_openai_models(
                SimpleNamespace(provider_type="openai", base_url=None), "sk"
            ),
            CURATED_PROVIDER_MODELS["openai"],
        )
    ...
    def test_fetch_openai_models_falls_back_when_transport_fails(self) -> None:
        ...
        with patch(
            "app.services.provider_service.httpx.Client",
            side_effect=httpx.HTTPError("boom"),
        ):
            self.assertEqual(
                service._fetch_openai_models(
                    SimpleNamespace(
                        provider_type="litellm", base_url="http://localhost:4000/v1"
                    ),
                    "sk",
                ),
    ...
    def test_fetch_openai_anthropic_gemini_and_ollama_fallback_when_base_url_missing(
        self,
    ) -> None:
        service = object.__new__(ProviderService)
        ...
        self.assertEqual(
            service._fetch_openai_models(
                SimpleNamespace(provider_type="custom", base_url=""), "sk"
            ),
            CURATED_PROVIDER_MODELS["openai"],
        )
        self.assertEqual(
            service._fetch_anthropic_models(
                SimpleNamespace(provider_type="custom", base_url=""), "sk"
            ),
            CURATED_PROVIDER_MODELS["anthropic"],
        )
        self.assertEqual(
            service._fetch_gemini_models(
                SimpleNamespace(provider_type="custom", base_url=""), "sk"
            ),
            CURATED_PROVIDER_MODELS["gemini"],
        )
        self.assertEqual(
            service._fetch_ollama_models(
                SimpleNamespace(provider_type="custom", base_url="")
            ),
    ...
    def test_fetch_anthropic_gemini_and_ollama_models_cover_remote_and_fallback_paths(
        self,
    ) -> None:
        service = object.__new__(ProviderService)
        ...
        with patch(
            "app.services.provider_service.httpx.Client",
            side_effect=[anthropic_client, gemini_client, ollama_client],
        ):
    ...
    def test_fetch_remote_model_helpers_cover_empty_and_invalid_payload_fallbacks(
        self,
    ) -> None:
        service = object.__new__(ProviderService)
        ...
        ollama_response = SimpleNamespace(
            raise_for_status=lambda: None,
            json=lambda: {"models": [{"model": None}, "skip"]},
        )
